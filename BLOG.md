...
| 项目 | 内容 |
|---|---|
| 这个作业属于哪个课程 | 软件工程（请补充课程链接） |
| 这个作业要求在哪里 | 软件工程课程第二次个人作业（请补充作业链接） |
...
项目共设计了 3 个难度递增的关卡。除了人工试玩外，还编写了深度优先搜索求解器和自动化测试，验证每个关卡至少存在一种通关顺序。
...
| 关卡设计 | Codex | 生成 3 个关卡数组，并提供可解性验证思路 | 初始布局存在循环阻挡、无法通关的风险。人工调整箭头位置和方向，并增加 DFS 求解器验证。最终 3 关均能自动求得完整消除顺序。 |
| 碰撞动画 | GitHub Copilot | 根据已有动画状态补全箭头晃动代码 | 初始代码基本可用，但速度较快。人工将动画设为 18 帧、每帧约 22 毫秒，并把最大位移控制在 8 像素；试玩时反馈更清晰，动画结束后能够恢复原位。 |
...
| T07 | 关卡可解性 | 对 3 个关卡运行 DFS 求解器 | 每关返回覆盖全部箭头的消除顺序 | 3 个关卡均找到完整解 | 通过 |
...
| 测试与修改 | 2.0 | 2.5 | +0.5 |
| README 与博客撰写 | 2.0 | 2.2 | +0.2 |
| **合计** | **15.0** | **17.2** | **+2.2** |

实际耗时高于预估 2.2 小时，主要原因是关卡可解性验证、边界问题检查和动画参数调整比预想耗时更多。
...
- [ ] 补充课程链接、作业链接和 GitHub 仓库链接；
- [ ] 本地运行全部自动化测试；
- [ ] 实际试玩 3 个关卡；
- [ ] 补充开始、游戏、通关和失败界面截图/GIF；
- [ ] 根据真实情况核对 PSP 实际耗时；
